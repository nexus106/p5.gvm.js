...
class GVM {
    ...
    static RECORD_KEY_PRESS_INTERVAL = 5;
    ...
    recordKeyPressTime() {
        ...
        this.keyPressTimes_ = this.keyPressTimes_.filter(time => currentTimeMs - time <= MAX_TIME_DIFF);

        if (this.keyPressTimes_.length >= RECORD_KEY_PRESS_INTERVAL) {
            let totalDiffMs = 0;
            for (let i = 1; i < RECORD_KEY_PRESS_INTERVAL; i++) {
                totalDiffMs += this.keyPressTimes_[this.keyPressTimes_.length - i] - this.keyPressTimes_[this.keyPressTimes_.length - i - 1];
            }

            const averageDiffMs = totalDiffMs / (RECORD_KEY_PRESS_INTERVAL - 1);
